...
def autobreadcrumbs_links(context):
    ...
    if 'autobreadcrumbs_elements' in context:
        ...
        for item in context['autobreadcrumbs_elements']:
            ...
            elements.append(settings.AUTOBREADCRUMBS_HTML_LINK.format(link=item.path, title=title))
        return mark_safe(settings.AUTOBREADCRUMBS_HTML_SEPARATOR.join(elements))
    return ''
